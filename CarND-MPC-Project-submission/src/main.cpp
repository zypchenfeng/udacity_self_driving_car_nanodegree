      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          for (unsigned int i = 0; i < ptsx.size(); i++) {
            double dx = ptsx[i] - px;
            double dy = ptsy[i] - py;
            ptsx_transform[i] = dx * cos(-psi) - dy * sin(-psi);
            ptsy_transform[i] = dx * sin(-psi) + dy * cos(-psi);
          }
          ...
          double pred_psi = -1.0 * ((v * delta * latency) / Lf);
          double pred_v = v + acceleration * latency;
          double pred_cte = cte + v * sin(epsi) * latency;
          double pred_epsi = epsi - ((v * delta * latency)/ Lf );

          Eigen::VectorXd state(6);
          //state << 0, 0, 0, v, cte, epsi;          
          state << pred_px, 0.0, pred_psi, pred_v, pred_cte, pred_epsi;          
